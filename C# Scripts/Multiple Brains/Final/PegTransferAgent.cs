...
public class PegTransferAgent : Agent
{
    ...
    public override void AgentAction(float[] act)
    {
        ...
        if (tip.transform.childCount == 1)
        {
            ...
            if (Mathf.Abs(targetDistZ) > 0.075f && (targetDistY < -0.25 && targetDistY > -0.35))
            {
                brain = 5;
                ConfigBrain(brain);
            ...
            if (Mathf.Abs(targetDistX) > 0.075f && (targetDistY < -0.25 && targetDistY > -0.35) && Mathf.Abs(targetDistZ) < 0.075f)
            {
                brain = 6;
                ConfigBrain(brain);
    ...
    private void ConfigBrain(int brain)
    {
        if (brain == 0)
        {
            GiveModel("NewGoalZ", NewGoalZ);
        }
        if (brain == 1)
        {
            GiveModel("NewGoalY", NewGoalY);
        }

        if (brain == 2)
        {
            GiveModel("NewGoalX", NewGoalX);
        }

        if (brain == 3)
        {
            GiveModel("NewTargetYUp", NewTargetYUp);
        }

        if (brain == 4)
        {
            GiveModel("NewTargetYDown", NewTargetYDown);
        }

        if (brain == 5)
        {
            GiveModel("NewTargetZ", NewTargetZ);
        }

        if (brain == 6)
        {
            GiveModel("NewTargetX", NewTargetX);
        }
    }
