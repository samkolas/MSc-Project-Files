...
public class PegTransferAgent : Agent
{
    ...
    public void Awake()
    {
        ...
        currentGoal = area.goal.transform.position;
        prevDist = currentGoal - tip.transform.position;
    ...
    public override void CollectObservations()
    {
        ...
        Vector3 goalDist = currentGoal - tip.transform.position; ;
    ...
    public override void AgentAction(float[] act)
    {
        ...
        PegTransferGoal goalScript = area.goal.GetComponent<PegTransferGoal>();
        ...
        if (tip.transform.childCount == 0)
        {
            ...
            Vector3 goalDist = currentGoal - tip.transform.position;
            if (Mathf.Abs(goalDist.x) < Mathf.Abs(prevDist.x) && Mathf.Abs(goalDist.z) < Mathf.Abs(prevDist.z) && Mathf.Abs(goalDist.y) < Mathf.Abs(prevDist.y) && goalDist.y < -0.1)
            {
                AddReward(0.4f);
            }
            else
        ...
        if (tip.transform.childCount == 1)
        {
            currentGoal = area.target.transform.GetChild(0).GetComponent<Transform>().position;
            float rotX = Mathf.Clamp(act[0] * rotMult * Time.fixedDeltaTime, -1, 1);
            float rotY = Mathf.Clamp(act[1] * rotMult * Time.fixedDeltaTime, -1, 1);
            ...
            Vector3 goalDist = currentGoal - tip.transform.position;
            if (Mathf.Abs(goalDist.x) < Mathf.Abs(prevDist.x) && Mathf.Abs(goalDist.z) < Mathf.Abs(prevDist.z) && Mathf.Abs(goalDist.y) < Mathf.Abs(prevDist.y) && goalDist.y > 0 && goalDist.y < 0.5f)
            {
                AddReward(0.4f);
            }
            else
