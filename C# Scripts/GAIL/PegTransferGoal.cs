...
public class PegTransferGoal : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (AInGoal)
        {
            ...
            float targetDistX = agentA.tip.transform.position.x - currentTarget.transform.GetChild(0).position.x;
            float targetDistZ = agentA.tip.transform.position.z - currentTarget.transform.GetChild(0).position.z;
            ...
            if (Mathf.Abs(targetDistX) < 0.2f && Mathf.Abs(targetDistZ) < 0.6f)
            {
